...
CREATE TABLE `wire_label`.`user_login` (
  `id` int NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`id`),
  KEY `user_id` (`user_id`),
  CONSTRAINT `user_login_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `wire_label`.`user` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_bin;


ALTER TABLE `wire_label`.`label` 
...
ADD UNIQUE INDEX `UC_pn_value_sort_rolling` (`part_number` ASC, `value` ASC, `sort_index` ASC, `rolling_label` ASC) VISIBLE,
ADD INDEX `label_ibfk_1_idx` (`modified_by_user_id` ASC) VISIBLE,
ADD INDEX `label_ibfk_2_idx` (`created_by_user_id` ASC) VISIBLE;
;
ALTER TABLE `wire_label`.`label` 
ADD CONSTRAINT `label_ibfk_1`
  FOREIGN KEY (`modified_by_user_id`)
  REFERENCES `wire_label`.`user` (`id`)
  ON DELETE NO ACTION
  ON UPDATE NO ACTION,
